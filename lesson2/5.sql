...
SELECT r.name region, s_r.name rep, a.name account
  FROM region r
  JOIN sales_reps s_r
    ON r.id = s_r.region_id
  JOIN accounts a
...
SELECT r.name AS RegionName, s_r.name AS SaleName, a.name AS AccountName
  FROM region r
  JOIN sales_reps s_r
    ON r.id = s_r.region_id
  JOIN accounts a
    ON s_r.id = a.sales_rep_id
 WHERE s_r.name LIKE 'S%' AND r.name = 'Midwest'
 ORDER BY a.name;
...
SELECT r.name AS RegionName, s_r.name AS SaleName, a.name AS AccountName
  ...
  JOIN accounts a 
    ON s_r.id = a.sales_rep_id
 WHERE s_r.name LIKE '% K%' AND r.name = 'Midwest'
 ORDER BY a.name;
